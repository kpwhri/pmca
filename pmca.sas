/*
    ...
    commercial or public administrative claims data or other sources of
    diagnostic coding data (e.g. electronic medical records; hospital
    discharge data) and to distinguish them from children with neither chronic
    nor chronic complex conditions (healthy children). 'Complex' and
    'Non-Complex' designations are assigned based on whether a child's
    condition(s) identified by ICD-9 and ICD-10 code(s) can be considered
    chronic, malignant, or progressive, and whether multiple body systems are
    involved.

    The AHRQ-funded Center of Excellence on Quality of Care Measures for
    Children with Complex Needs developed consensus definitions for children
    with complex chronic disease (C-CD), children with non-complex chronic
    disease (NC-CD), and children without CD (healthy). Development and
    testing of this PMCA was supported by the COE (Rita Mangione-Smith, MD
    MPH, Principal Investigator).

    To assign involved body system(s) and progressive status, Seattle
    Children's Research Institute evaluated conditions defined by diagnostic
    codes outlined in the Chronic Illness and Disability Payment System (CDPS)
    version 5.3, and identified conditions of interest based on their chronic
    nature.
  ...
  Input Source Data:
    ...
    have multiple lines, and a single record/line may contain a single field
    or multiple fields for diagnosis codes. ICD-9 and ICD-10 codes may be
    intermixed within a single patient but not within a single encounter. The
    program identifies whether an encounter's codes are ICD-9 or ICD-10
    format. Then the program rolls up assignments by claim and sets an array
    for ICD-9 or ICD-10 codes, so that it accommodates source files that have
    a different number of possible diagnosis code fields, or source files that
    have one row for each diagnosis code.

    Format for ICD-9 or ICD-10 codes:

    This program assumes that incoming ICD-9 or ICD-10 codes (as the variable
    called 'icdvar') are in xxx.xx character format with leading zeros for
    codes that do not have 3 characters before the decimal. The program then
    strips out the decimals.  If original source data do not have decimals,
    the stripping command can be deleted from the processing.

    The program is designed to process records with a single diagnosis code
    field or diagnosis code fields. A macro statement has been provided at the
    start of the code to set the number of fields. To change the number of
    diagnosis fields processed, change 'icdnum' in the SET VALUES FOR
    VARIABLES section to the number of fields in a single record of the source
    data.

  The Process:

    1) 'Claims' dataset:  claims are read in and the decimals are stripped
        from the ICD-9 or ICD-10 codes. If incoming data do not have decimals, the
        do-end 'compress' segment can be commented out. ICD-9 or ICD-10 codes with
        or without decimals are thus accommodated.
    ...
    4)  'Flagclaims' processing steps through the multiple lines (or single
        line) of individual claims for a person, identifying body systems
        involved, progressive status of a condition, and malignancy. The final
        result is a dataset where identifications of body system, progressivity,
        and malignancy have been rolled up to a single record per claim, with a
        single indication of any specified occurrence (i.e. once per claim).
  ...
  Final Condition Definitions:
    ...
    assignments for 'Complex Chronic', 'Non-complex Chronic', and
    'Non-Chronic'.

    The variable 'cond_less' contains values from the less conservative
    algorithm, and is designed to be used with less detailed data sources,
    such as those without outpatient claims.

    The variable 'cond_more' contains values from the more conservative
    algorithm, and is designed to be used with more detailed data sources such
    as those including inpatient and outpatient claims.
...
%macro classify_dx(dx_varname, dx_codetype = 09) ;
  ...
  if &dx_codetype = 09 then do ;
    ...
    if &dx_varname in: ('F7' 'F801' 'F802' 'F804' 'F81' 'F82'
    ...
    if &dx_varname in: ('F842' 'G09' 'G10' 'G111' 'G113'
      'G114' 'G118' 'G119' 'G12'
    ...
    * [RP] Adding a space at the end of M358 b/c we *do* want M35.8 but we dont want M35.81 ;
    ...
    if &dx_varname in: ('P270' 'P271' 'P278' )   then pulresp=1;
    ...
    if &dx_varname in: ('Q210' 'Q211' 'Q220' 'Q221' 'Q222'
...
data flagclaims(keep=&sid &claimid
  ...
  * preset all the flags to 0 for rollup to one record with designations per claim;
  ...
  if icd10codeset=0 then
    do i = 1 to &icdnum;
      %classify_dx(dx_varname = &icdvar{i}, dx_codetype = 09) ;
    end ;

  else if icd10codeset=1 then
    do i = 1 to &icdnum;
      %classify_dx(dx_varname = &icdvar{i}, dx_codetype = 10) ;
    end;
...
data oput.results_pmca(keep=&sid
  ...
  if last.&sid then
    do;
      ...
      *******************************************************************************;
        ...
        * [RP] will we have a missing value propagation error here? Maybe switch to sum()? ;
